...
/**
 * Canonical CBOR map from [key, encodedValue] pairs.
 *
 * Canonical ordering sorts keys by encoded length first, then bytewise — the
 * same rule the runtime applies, so a map built here hashes identically.
 */
export const map = (entries) => {
  const encoded = entries.map(([key, value]) => [text(key), value]);
  encoded.sort(([a], [b]) => (a.length - b.length)
